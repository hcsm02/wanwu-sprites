...
FALLBACK_ACTIVITIES: list[ActivityCard] = [
    ActivityCard(
        ...
        steps=["找一面有光的墙", "你做一个简单影子", "换孩子做，你来模仿"],
        question="哪个影子最好笑？",
...
def recommend_activity(context: ActivityContext) -> ActivityCard:
    ...
    prompt = f"""
你是一个亲子微活动推荐助手，产品角色叫“玩精灵”。
请根据当前场景生成 1 个低门槛亲子活动。
...
def create_memory(activity_title: str, mood: str, one_line_note: str) -> str:
    ...
    client = _get_client()
    if client is None:
        return f"今天我们一起完成了《{activity_title}》。孩子的状态是“{mood}”。{clean_note} 这是一个简单但值得保存的亲子时刻。"
